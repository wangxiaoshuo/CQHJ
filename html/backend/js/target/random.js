        ...
        if ( pcount >= Randomnumber ) {
            stopTime = window.setInterval(function () {
                if (runing) {
                    ...
                    // }else{
                        //打印抽取者名单
                        $('.luck-user-list').prepend('<li><div class="portrait" style="background-image:url('+xinm[num]+')"></div><div class="luckuserName">'+info[num].name+'</div><div class="luckuserPhone">'+info[num].phone+'</div><a class="luckuserbtn test'+num+'" data-index="'+info[num].phone+'" onclick="remarkModal(this)" href="javascript:;" title="有事"><i class="Hui-iconfont">&#xe60b;</i></a></li>')
